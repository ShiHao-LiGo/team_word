  ...
  methods: {
    ...
    async search() {
      await HTTP.get(this.url).then((response) => {
        ...
        //
        this.relationshipAnnotations = this.docs.map(doc => doc.relationshipAnnotations);
        //
...
//
    removeRelationshipAnnotation(relationshipAnnotation) {
      console.log('this.id2relationshipAnnotationIndex');
      console.log(this.id2relationshipAnnotationIndex);
      const docId = this.docs[this.pageNumber].id;
      console.log('想要删除:');
      const index = this.id2relationshipAnnotationIndex[relationshipAnnotation.id];
      console.log(relationshipAnnotation.id + ' to ' + index);
      this.relationshipAnnotations[this.pageNumber].splice(index,1);
      return HTTP.delete(`docs/${docId}/relationshipAnnotations/${relationshipAnnotation.id}`).then(() => {
        // const index = this.annotations[this.pageNumber].indexOf(annotation);
        // this.annotations[this.pageNumber].splice(index, 1);
      });
    },
//
    ...
    // 提交后台
    approveDocumentAnnotations_relationships() {
      const document = this.docs[this.pageNumber];
      const approved = !this.documentAnnotationsAreApproved;

      HTTP.post(`docs/${document.id}/approve-relationships`, { approved }).then((response) => {
        const documents = this.docs.slice();
        documents[this.pageNumber] = response.data;
        this.docs = documents;
      });
