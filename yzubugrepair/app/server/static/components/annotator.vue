<template lang="pug">
  div(@click="setSelectedRange", @mousemove="Mousemove($event)")
    //- svg
    span.text-sequence(

      v-for="r in chunksWithLabel"
      @mousedown="r.label != -1 && labelAnotationMousedown(r,$event)"
  ...
  methods: {
    ...
    addLabel(labelId) {
      if (this.validRange()) {
        ...
        console.log('annotator.vue : addLabel()')
    ...
    labelAnotationMousedown(r,event){
      console.log('r = ');
        console.log(r);
        console.log('标注的文本为');
        console.log(this.text.substring(r.start_offset, r.end_offset))
        if(this.fromLabel == -1){
            this.preSequenceText = this.text.substring(r.start_offset, r.end_offset);
            this.fromLabel = r.id;
            console.log('first click labelId :' + this.fromLabel );
        }else if(this.toLabel == -1){
            this.fowlSequenceText = this.text.substring(r.start_offset, r.end_offset);

            this.toLabel = r.id;

            const relationship = {
              precursor_sequenceAnnotation: this.fromLabel,
              follow_sequenceAnnotation: this.toLabel,
              relationship: -1,
            };            

            this.fromLabel = -1;
            this.toLabel = -1;
            this.$parent.tryAddRelationship(relationship, this.preSequenceText, this.fowlSequenceText);
            // this.$parent.addRelationship(relationship);

        }
    },

    labelAnotationMouseup(obj){
         
         console.log('==============  Mouseup labelAnotation =========');
