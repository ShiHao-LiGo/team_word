...
from .models import Project, Label, Document, RoleMapping, Role, Relationship
...
from .serializers import ProjectSerializer, LabelSerializer, RelationshipSerializer, DocumentSerializer, UserSerializer, ApproverSerializer
...
class StatisticsAPI(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not include or 'relationship' in include:
            relationship_count, user_count = self.relationship_per_data(p)
            response['relationship'] = relationship_count
            # TODO: Make user_relationship count chart
            response['user_relationship'] = user_count
    ...
    def relationship_per_data(self, project):
        annotation_class = project.get_annotation_class()
        return annotation_class.objects.get_relationship_per_data(project=project)
...
class ApproveRelationshipsAPI(APIView):
    permission_classes = [IsAuthenticated & (IsAnnotationApprover | IsProjectAdmin)]
...
class AnnotationList(generics.ListCreateAPIView):
    ...
    def check_single_class_classification(project_id, doc_id, user):
        print('------  check_single_class_classification()  -----')
...
class RelationshipAnnotationList(generics.ListCreateAPIView):
    pagination_class = None
    permission_classes = [IsAuthenticated & IsInProjectOrAdmin]
    swagger_schema = None
    ...
    def get_serializer_class(self):
        print('--- get_serializer_class() -----')
        project = get_object_or_404(Project, pk=self.kwargs['project_id'])
        self.serializer_class = project.get_relationship_annotation_serializer()
        print('self.serializer_class : ')
        print(self.serializer_class)
        print('----------------')
        return self.serializer_class
    ...
    def get_queryset(self):
        project = get_object_or_404(Project, pk=self.kwargs['project_id'])
        model = project.get_relationship_annotation_serializer()
    ...
    def check_single_class_classification(project_id, doc_id, user):
        ...
        model = project.get_relationship_annotation_class()
        RelationshipAnnotations = model.objects.filter(document_id=doc_id)

        print(RelationshipAnnotations)
        print('---------------')
        if not project.collaborative_annotation:
            RelationshipAnnotations = RelationshipAnnotations.filter(user=user)

        if RelationshipAnnotations.exists():
            raise ValidationError('requested to create duplicate annotation for single-class-classification project')
...
class RelationshipAnnotationDetail(generics.RetrieveUpdateDestroyAPIView):
    lookup_url_kwarg = 'relationshipAnnotation_id'
    swagger_schema = None
...
class TextUploadAPI(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        print('------ TextUploadAPI.post: request.data ------- ')
        print(request.data)
        self.save_file(
            user=request.user,
...
class CloudUploadAPI(APIView):
    ...
    def get(self, request, *args, **kwargs):
        print('-----------CloudUploadAPI.get()000000000')
        try:
...
####################
class RelationshipUploadAPI(APIView):
    parser_classes = (MultiPartParser,)
    permission_classes = [IsAuthenticated & IsProjectAdmin]
    ...
    def post(self, request, *args, **kwargs):
        if 'file' not in request.data:
            raise ParseError('Empty content')
        relationships = json.load(request.data['file'])
        print(relationships)
        project = get_object_or_404(Project, pk=kwargs['project_id'])
        try:
            for relationship in relationships:
                serializer = RelationshipSerializer(data=relationship)
                serializer.is_valid(raise_exception=True)
                serializer.save(project=project)
            return Response(status=status.HTTP_201_CREATED)
        except IntegrityError:
            content = {'error': 'IntegrityError: you cannot create a relationship with same name or shortkey.'}
            return Response(content, status=status.HTTP_400_BAD_REQUEST)
