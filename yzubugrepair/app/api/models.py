...
class Project(PolymorphicModel):
    ...
    #####
    def get_relationship_annotation_serializer(self):
        raise NotImplementedError()

    def get_relationship_annotation_class(self):
        raise NotImplementedError()
...
####
    def get_relationship_annotation_serializer(self):
        from .serializers import RelationshipAnnotationSerializer
        return RelationshipAnnotationSerializer
...
class Relationship(models.Model):
    ...
    prefix_key = models.CharField(max_length=10, blank=True, null=True, choices=PREFIX_KEYS)
    suffix_key = models.CharField(max_length=1, blank=True, null=True, choices=SUFFIX_KEYS)
    project = models.ForeignKey(Project, related_name='relationships', on_delete=models.CASCADE)
    background_color = models.CharField(max_length=7, default='#209cee')
    text_color = models.CharField(max_length=7, default='#ffffff')
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
...
class RelationshipAnnotation(Annotation):
    document = models.ForeignKey(Document, related_name='relationship_annotations', on_delete=models.CASCADE)
    relationship = models.ForeignKey(Relationship, on_delete=models.CASCADE, null=True)
    precursor_sequenceAnnotation = models.ForeignKey(SequenceAnnotation, on_delete=models.CASCADE, related_name="precursor_sequenceAnnotation", null=True)
    follow_sequenceAnnotation = models.ForeignKey(SequenceAnnotation, on_delete=models.CASCADE, related_name="follow_sequenceAnnotation", null=True)
