...
from .serializers import DocumentSerializer, LabelSerializer, RelationshipSerializer
...
class BaseStorage(object):
    ...
    def save_relationship(self, data):
        serializer = RelationshipSerializer(data=data, many=True)
        serializer.is_valid(raise_exception=True)
        relationship = serializer.save(project=self.project)
        return relationship
    ...
    def save_relationship_annotation(self, data, user):
        relationship_annotation_serializer = self.project.get_relationship_annotation_serializer()
        serializer = relationship_annotation_serializer(data=data, many=True)
        serializer.is_valid(raise_exception=True)
        relationship_annotation = serializer.save(user=user)
        return relationship_annotation
    ...
    @classmethod
    def extract_relationship(cls, data):
        return [d.get('relationships', []) for d in data]
    ...
    @classmethod
    def exclude_created_relationships(cls, relationships, created):
        return [relationship for relationship in relationships if relationship not in created]
    ...
    @classmethod
    def to_serializer_relationship_format(cls, relationships, created):
        # existing_shortkeys = {(label.suffix_key, label.prefix_key)
        #                       for label in created.values()}
        ...
        for relationship in sorted(relationships):
            serializer_relationship = {'text': relationship}
            ...
            background_color = Color(pick_for=relationship)
            text_color = Color('white') if background_color.get_luminance() < 0.5 else Color('black')
            serializer_relationship['background_color'] = background_color.hex
            serializer_relationship['text_color'] = text_color.hex

            serializer_relationships.append(serializer_relationship)

        return serializer_relationships
    ...
    @classmethod
    def update_saved_relationships(cls, saved, new):
        for relationship in new:
            saved[relationship.text] = relationship
        return saved
...
class SequenceLabelingStorage(BaseStorage):
    ...
    def save(self, user):
        saved_labels = {label.text: label for label in self.project.labels.all()}
        saved_relationships = {relationship.text: relationship for relationship in self.project.relationships.all()}
        for data in self.data:
            docs = self.save_doc(data)
            labels = self.extract_label(data)
            unique_labels = self.extract_unique_labels(labels)
            ...
            relationships = self.extract_relationship(data)

            unique_relationships = self.extract_unique_relationships(relationships)
            unique_relationships = self.exclude_created_relationships(unique_relationships, saved_relationships)
            unique_relationships = self.to_serializer_relationship_format(unique_relationships, saved_relationships)

            new_relationships = self.save_relationship(unique_relationships)

            saved_relationships = self.update_saved_relationships(saved_relationships, new_relationships)

            relationships_annotations = self.make_relationship_annotations(docs, relationships, saved_relationships)
            self.save_relationship_annotation(relationships_annotations, user)
    ...
    @classmethod
    def extract_unique_relationships(cls, relationships):
        return set([relationship for _, _, _, _, _, _, relationship in itertools.chain(*relationships)])
    ...
    @classmethod
    def start_end_lable2id(cls, seq_antt):
        start_end_lable_id = {}
        for s in seq_antt:
            start_end_lable_id[str(s.start_offset) + str(s.end_offset) + str(s.label)] = s.id
        return start_end_lable_id

    @classmethod
    def make_relationship_annotations(cls, docs, relationships, saved_relationships):
        relationship_annotations = []
        for doc, spans in zip(docs, relationships):
            sequnce_annotations = {sqc_antt: sqc_antt for sqc_antt in doc.seq_annotations.all()}

            lable_id = cls.start_end_lable2id(sequnce_annotations)
            for span in spans:
                pre_start_offset, pre_end_offset, pre_name,\
                    flow_start_offset, flow_end_offset, flow_name, \
                    relationshipType = span
                relationship = saved_relationships[relationshipType]
                relationship_annotations.append({'document': doc.id,
                                    'relationship': relationship.id,
                                    'precursor_sequenceAnnotation': lable_id[str(pre_start_offset) + str(pre_end_offset) + pre_name],
                                    'follow_sequenceAnnotation': lable_id[str(flow_start_offset) + str(flow_end_offset) + flow_name]})
        return relationship_annotations
