...
from .models import Label, Relationship, Project, Document, RoleMapping, Role, RelationshipAnnotation
...
class RelationshipSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        try:
            context = self.context['request'].parser_context
            project_id = context['kwargs']['project_id']
            relationship_id = context['kwargs'].get('relationship_id')
        except (AttributeError, KeyError):
            pass  # unit tests don't always have the correct context set up
        else:
            conflicting_relationships = Relationship.objects.filter(
                suffix_key=suffix_key,
                prefix_key=prefix_key,
                project=project_id,
            )
            ...
            if relationship_id is not None:
                conflicting_relationships = conflicting_relationships.exclude(id=relationship_id)

            if conflicting_relationships.exists():
                raise ValidationError('Duplicate shortcut key.')
...
class DocumentSerializer(serializers.ModelSerializer):
    ...
    ####
    relationshipAnnotations = serializers.SerializerMethodField()
    relationshipAnnotation_approver = serializers.SerializerMethodField()
    ###
    ...
    #######
    def get_relationshipAnnotations(self, instance):
        request = self.context.get('request')
        project = instance.project
        model = project.get_relationship_annotation_class()
        serializer = project.get_relationship_annotation_serializer()
        relationshipAnnotation = model.objects.filter(document=instance.id)
        if request and not project.collaborative_annotation:
            relationshipAnnotation = relationshipAnnotation.filter(user=request.user)
        serializer = serializer(relationshipAnnotation, many=True)
        return serializer.data
    ########
    @classmethod
    ...
    @classmethod
    def get_relationshipAnnotation_approver(cls, instance):
        approver = instance.annotations_approved_by
        return approver.username if approver else None
    ...
    class Meta:
        model = Document
        fields = ('id', 'text', 'annotations', 'meta', 'annotation_approver', 'relationshipAnnotations', 'relationshipAnnotation_approver')
...
class RelationshipAnnotationSerializer(serializers.ModelSerializer):
    #label = ProjectFilteredPrimaryKeyRelatedField(queryset=Label.objects.all())
    relationship = serializers.PrimaryKeyRelatedField(queryset=Relationship.objects.all())
    document = serializers.PrimaryKeyRelatedField(queryset=Document.objects.all())

    class Meta:
        model = RelationshipAnnotation
        fields = ('id', 'prob', 'relationship', 'precursor_sequenceAnnotation', 'follow_sequenceAnnotation', 'user', 'document', 'created_at', 'updated_at')
        read_only_fields = ('user',)
###
